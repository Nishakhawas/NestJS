...
export default function FrontendMenuList() {

    const queryClient = useQueryClient();

    const fetchMenuList = async () => {
        const res = await axios.get('http://localhost:3000/tiles');
        return res.data;
    };
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => {
            ...
            return axios.delete(`http://localhost:3000/tiles/${id}`, {
        ...
        onSuccess: () => {
            toast.success("Frontend Menu deleted Successfully", {
    ...
    console.log("🚀 ~ FrontendMenuList ~ filters:", filters)
    ...
    const { data: searchedData } = useQuery({
        ...
        queryFn: async () => {
            if (isGlobalSearchActive && !isColumnSearchActive) {
                // Global Search only
                const res = await axios.get('http://localhost:3000/tiles/search', {
                    params: { search: searchText },
                });
                return res.data;
            } else if (isColumnSearchActive && !isGlobalSearchActive) {
                // Column filters only
                const res = await axios.get('http://localhost:3000/tiles/columnquery', {
                    params: filters,
                });
                return res.data;
                ...
                const [global, column] = await Promise.all([
                    axios.get('http://localhost:3000/tiles/search', { params: { search: searchText } }),

                    axios.get('http://localhost:3000/tiles/columnquery', { params: filters }),
                ]);
    ...
    const columns = [
...

        {
            ...
            cell: row => row.isPlaceholder ? '' : row.icon,
...

        {
            ...
            width: "170px",
            cell: (row) => (
                console.log("🚀 ~ MenuList ~ row.imageUrl:", row.imageUrl),
                row.isPlaceholder ? '' :
                    <div className='flex flex-col'>
                        <img
...

        {
            ...
            cell: row => row.isPlaceholder ? '' : row.content,
            sortable: true
...

        {
            ...
            cell: row => row.isPlaceholder ? '' : row.order,
            sortable: true
        },
            ...
            < div className='max-w-6xl mt-10 mx-auto flex flex-col gap-2 bg-white '>
                <div className='bg-sky-300 flex justify-between items-center p-2'>
                    <div className='flex-1 text-center'>
                        <h2 className='font-medium text-gray-500'>Frontend Tiles List</h2>
                    </div>
                    <div className='flex justify-end'>
                        <button
                            onClick={() => setShowCreateModal(true)} className='p-1 text-white font-semibold bg-sky-600 hover:bg-sky-700 hover:shadow-sky-100 justify-end'> + Create Tiles</button>
                    </div>
                </div>
                ...
                {showCreateModal && (
                    ...
                    >
                        ...
                        >
                            {/*  Close button */}
                            <button
                                className="absolute top-14 right-8 text-gray-500 hover:text-red-500 text-xl font-bold"
